...
def findMinTheta( theta, X, y, lamda ):
    ops = {"maxiter":500, "disp":True}
    result = scipy.optimize.minimize( costFunction, theta, args=(X, y, lamda),  method='BFGS', options=ops )
...
def part2_3(data):
    ...
    lamda = 1.0
    test_cost = computeCost( theta, X, y, lamda )
    print("computeCost =", test_cost)
    theta, cost = findMinTheta( theta, X, y, lamda )
    print("finishCost =", cost)
    print("resTheta =", theta)
...
def main():
    set_printoptions(threshold=10, linewidth=120, precision=6)
    ex2data  = genfromtxt("ex2data2.txt", delimiter=',')
    # part2_1(ex2data)
    # part2_2(ex2data)
    part2_3(ex2data)
    # part2_4(ex2data)
