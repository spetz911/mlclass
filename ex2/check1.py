...
def findMinTheta( theta, X, y ):
    result = scipy.optimize.fmin( costFunction, x0=theta, args=(X, y), maxiter=400, full_output=True )
...
def part1_2():
    ...
    print(" computeCost(theta, X, y)",  computeCost(theta, X, y))
    ...
    test = array([1, 45, 85])
    print("predict( test, theta )", predict( test, theta ))
